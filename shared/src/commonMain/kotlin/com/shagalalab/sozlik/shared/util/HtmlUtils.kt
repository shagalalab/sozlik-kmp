...
fun String.parseHtml(): AnnotatedString {
    ...
    return buildAnnotatedString {
        recourse(newlineReplace, this)
    }
}

/**
 * Recourses through the given HTML String to convert it to an AnnotatedString.
 *
 * @param string the String to examine.
 * @param to the AnnotatedString to append to.
 */
private fun recourse(string: String, to: AnnotatedString.Builder) {
    ...
    when {
        ...
        // SpanStyle and continue recoursing.
        tags.any { string.startsWith(it.value) } -> {
            to.pop()
            recourse(string.removeRange(0, endTag!!.length), to)
        }
        ...
        tags.any { string.startsWith(it.key) } -> {
            ...
            recourse(string.removeRange(0, startTag.length), to)
        }
        ...
        tags.any { string.contains(it.key) || string.contains(it.value) } -> {
            ...
            recourse(string.removeRange(0, first), to)
        }
